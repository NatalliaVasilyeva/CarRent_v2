...
<head>
    ...
    <jsp:include page="/WEB-INF/include/i18n.html"/>
        <script type="text/javascript" src="${pageContext.request.contextPath}/js/order/admin-order.js"></script>
...
<c:if test="${empty orders}">
    <h1 align="center">There are not orders!</h1>
</c:if>
<c:if test="${not empty orders}">
                                            ...
                                            <tr class="info">
                                                ...
                                                <c:choose>
                                                    <c:when test="${order.orderStatus eq 'CONFIRMATION_WAIT'}">
                                                        <td>
                                                            <form id="confirm-order-form" class="form-horizontal" method="POST" action="${pageContext.request.contextPath}/admin-change-status-order">
                                                                <input type="text" name="orderId" value="${order.id}" hidden>
                                                                <input type="text" name="orderStatus" value="confirm" hidden>
                                                                <button type="submit" class="btn btn-dark btn-xs" value="Submit">
                                                                    <i class="fa fa-check-square-o" aria-hidden="true"></i>
                                                                </button>
                                                            </form>
                                                        </td>
                                                        <td>
                                                            <form id="decline-order-form" class="form-horizontal" method="POST" action="${pageContext.request.contextPath}/admin-change-status-order">
                                                                <input type="text" name="orderId" value="${order.id}" hidden>
                                                                <input type="text" name="orderStatus" value="decline" hidden>
                                                                <button type="submit" class="btn btn-dark btn-xs" value="Submit">
                                                ...
                                                <c:choose>
                                                    <c:when test="${order.orderStatus eq 'NOT_PAYED'}">
                                                        <td>
                                                            <form id="payed-order-form" class="form-horizontal" method="POST" action="${pageContext.request.contextPath}/admin-change-status-order">
                                                                <input type="text" name="orderId" value="${order.id}" hidden>
                                                                <input type="text" name="orderStatus" value="payed" hidden>
                                                                <button type="submit" class="btn btn-dark btn-xs" value="Submit">
                                                ...
                                                <c:choose>
                                                    <c:when test="${order.orderStatus eq 'PAYED'}">
                                                        <td>
                                                            <button class="btn btn-dark btn-xs addaccidentbtn"
                                                                    data-toggle="modal"
                                                                    data-target="#add-accident-modal">
                                                                <i class="fa fa-plus" aria-hidden="true"><fmt:message
                                                                        key="form.create_accident"/></i>
                                                            </button>
                                                        </td>
                                                    </c:when>
                                                </c:choose>
    ...
    <!-- Attachment Modal -->
    <div class="modal fade" id="add-accident-modal" tabindex="-1" role="dialog" aria-labelledby="edit-modal-label"
         aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="edit-modal-label">Add accident</h5>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary" form="add-accident-form" value="Submit"><fmt:message
                            key="form.submit_button"/></button>
