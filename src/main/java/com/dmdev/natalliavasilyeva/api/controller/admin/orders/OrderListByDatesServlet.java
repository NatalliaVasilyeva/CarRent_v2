...
@WebServlet("/admin-orders-by-dates")
public class OrderListByDatesServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if (!dataValidator.isValidData(VariablesNameHolder.BIRTHDAY, dateFrom) || !dataValidator.isValidData(VariablesNameHolder.BIRTHDAY, dateTo)) {
                ...
                LocalDate localDateFrom = DateTimeService.fromStringLocalDate(dateFrom);
                LocalDate localDateTo = DateTimeService.fromStringLocalDate(dateTo);
                List<OrderResponseDto> orders = OrderMapper.toDtos(orderService.findAllCustomOrdersBetweenDates
                        (DateTimeService.fromLocalDateTimeDateToInstant(localDateFrom.atStartOfDay()),
                                DateTimeService.fromLocalDateTimeDateToInstant(localDateTo.atTime(LocalTime.MAX))));
